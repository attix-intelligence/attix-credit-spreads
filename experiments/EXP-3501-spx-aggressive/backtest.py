"""
EXP-3501: SPX 0DTE Aggressive (20Δ) with REAL CBOE Data
...
Strategy: Aggressive 20Δ iron condor (tighter strikes, higher premium)
Ticker: SPX (0DTE Mon/Wed/Fri)
Period: 2021-2025 (full CBOE dataset)

North Star: Path A Pillar 2 - TARGET STRATEGY
Goal: 30-50% monthly returns, Sharpe >2.0
"""
...
EXPERIMENT_ID = "EXP-3501"
TICKER = "SPX"
START_DATE = "2021-01-04"  # First trading day of 2021
END_DATE = "2025-05-23"    # Latest CBOE data

# Strategy parameters (AGGRESSIVE)
SHORT_DELTA_TARGET = 0.20  # 20Δ strikes (MORE AGGRESSIVE than 30Δ baseline)
...
def find_delta_strike(
    ...
    # Query Greeks for candidate strikes (sample 10 around center)
    strikes_sorted = sorted(strikes)
    mid_idx = len(strikes_sorted) // 2
    start = max(0, mid_idx - 5)
    end = min(len(strikes_sorted), mid_idx + 5)
...
def execute_iron_condor(
    ...
    # Find 20Δ short strikes using REAL CBOE Greeks (MORE AGGRESSIVE)
    ...
    put_credit = put_spread_prices['short_bid'] - put_spread_prices['long_ask']
    call_credit = call_spread_prices['short_bid'] - call_spread_prices['long_ask']
    ...
    # For now, use EOD prices to determine if profit target hit or stopped out
    ...
    if not exit_prices_put or not exit_prices_call:
        ...
        put_exit_cost = exit_prices_put['short_ask'] - exit_prices_put['long_bid']
        call_exit_cost = exit_prices_call['short_ask'] - exit_prices_call['long_bid']
...
def run_backtest():
    """Run full backtest from 2021-2025."""
    logger.info(f"{'='*60}")
    logger.info(f"Starting {EXPERIMENT_ID}: SPX 0DTE Aggressive (20Δ)")
    logger.info(f"Period: {START_DATE} to {END_DATE}")
    logger.info(f"Data: CBOE Athena (RULE ZERO COMPLIANT)")
    logger.info(f"North Star: Path A Pillar 2 TARGET STRATEGY")
    logger.info(f"{'='*60}\n")
    ...
    df_equity = pd.DataFrame({
        'date': [START_DATE] + [t['date'] for t in trades],
        'equity': equity_curve
    })
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Target: 30-50% monthly returns, Sharpe >2.0")
    ...
    if avg_monthly_return >= 0.30 and sharpe_ratio >= 2.0:
        logger.info(f"✅ WINNER - Meets Path A Pillar 2 targets!")
    elif avg_monthly_return >= 0.20 and sharpe_ratio >= 1.5:
        logger.info(f"⚠️  PROMISING - Close to targets, needs tuning")
    else:
    ...
    results = {
        ...
        "north_star": "Path_A_Pillar_2_TARGET_STRATEGY",
        "parameters": {
