...
def load_day_data(date: datetime) -> pd.DataFrame:
    ...
    # Parse timestamps
    df['timestamp'] = pd.to_datetime(df['quote_datetime'])
    
    # Add SPX underlying price (use ATM call for reference)
    atm_calls = df[(df['option_type'] == 'C') & (df['delta'].between(0.45, 0.55))]
    if not atm_calls.empty:
        underlying = atm_calls.iloc[0]['underlying_last']
    else:
        underlying = df['underlying_last'].median()
    
    df['underlying'] = underlying
    
    return df

def find_strike_by_delta(df: pd.DataFrame, entry_time: time, option_type: str, target_delta: float, underlying: float) -> dict:
    """
    Find the strike closest to target delta at entry time.
    
    Returns dict with: strike, delta, bid, ask, mid
    """
    # Filter to entry time window (±5 minutes)
    entry_dt = datetime.combine(df['timestamp'].iloc[0].date(), entry_time)
    df_entry = df[
        (df['timestamp'] >= entry_dt - timedelta(minutes=5)) &
        (df['timestamp'] <= entry_dt + timedelta(minutes=5)) &
...
def get_strike_price(df: pd.DataFrame, strike: float, option_type: str, target_time: time) -> dict:
    ...
    df_strike = df[
        (df['timestamp'] >= target_dt - timedelta(minutes=5)) &
        (df['timestamp'] <= target_dt + timedelta(minutes=5)) &
        (df['strike'] == strike) &
        (df['option_type'] == option_type)
    ]
...
def run_backtest():
    ...
    for i, date in enumerate(trading_days, 1):
        ...
        # Find 20Δ strikes
        put_short = find_strike_by_delta(df, ENTRY_TIME, 'P', TARGET_DELTA_PUT, underlying)
        call_short = find_strike_by_delta(df, ENTRY_TIME, 'C', TARGET_DELTA_CALL, underlying)
