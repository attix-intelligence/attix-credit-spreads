"""
EXP-3504: SPX 0DTE with XLK IV Filter - REAL CBOE Data
...
This was the QQQ winner (Sharpe 2.10, avoided 3/5 tech events)
Validate on SPX.
"""
...
# XLK IV Filter Strategy parameters
VIX_FILTER_THRESHOLD = 25.0  # Skip trade if VIX > 25% (tech sector risk)
BASE_DELTA = 0.20  # 20Δ (aggressive baseline - always use this when trading)
...
def should_skip_trade(vix_level: float) -> Tuple[bool, str]:
    """
    Determine if trade should be skipped based on VIX (XLK IV proxy).
    Returns: (skip, reason)
    """
    if vix_level > VIX_FILTER_THRESHOLD:
        return True, f"VIX {vix_level:.1f} > {VIX_FILTER_THRESHOLD} (tech sector risk)"
    return False, ""
...
def construct_iron_condor(
    ...
    call_credit = call_spread['short_bid'] - call_spread['long_ask']
    put_credit = put_spread['short_bid'] - put_spread['long_ask']
...
def get_spread_value(
    ...
    # Debit to close (use conservative: buy back at ask, sell longs at bid)
    call_debit = call_spread['short_ask'] - call_spread['long_bid']
    put_debit = put_spread['short_ask'] - put_spread['long_bid']
...
def run_backtest():
    ...
    logger.info(f"Base Delta: {BASE_DELTA:.0%} (aggressive baseline)")
    ...
    while current_date <= end:
        ...
        # Check filter: Skip if VIX too high
        skip, reason = should_skip_trade(vix_level)
        if skip:
            logger.info(f"{date_str}: SKIPPED - {reason}")
            trades_filtered += 1
            current_date += timedelta(days=1)
            continue
        
        logger.info(f"{date_str}: VIX={vix_level:.2f}, TRADING with {BASE_DELTA:.0%} delta")
        ...
        # Construct iron condor with base delta (always 20Δ when we trade)
        ic = construct_iron_condor(
            provider, date_str, expiration, underlying, BASE_DELTA
        )
    ...
    logger.info(f"Trades filtered: {trades_filtered}")
    logger.info(f"Filter rate: {trades_filtered/(len(trades)+trades_filtered)*100:.1f}%")
    logger.info(f"Final capital: ${current_capital:,.2f}")
    logger.info(f"Total return: {((current_capital / CAPITAL - 1) * 100):.2f}%")
    
    # Calculate metrics
    calculate_metrics(trades_df, equity_df, trades_filtered)


def calculate_metrics(trades_df: pd.DataFrame, equity_df: pd.DataFrame, trades_filtered: int):
    ...
    # Average VIX on traded days
    avg_vix_traded = trades_df['vix'].mean()
    ...
    logger.info(f"Trades Filtered: {trades_filtered}")
    logger.info(f"Filter Rate: {trades_filtered/(total_trades+trades_filtered)*100:.1f}%")
    logger.info(f"Avg VIX (traded days): {avg_vix_traded:.1f}")
    logger.info(f"\nPERFORMANCE:")
    ...
    metrics = {
        ...
        'trades_filtered': int(trades_filtered),
        'filter_rate_pct': float(trades_filtered/(total_trades+trades_filtered)*100),
        'avg_vix_traded': float(avg_vix_traded),
