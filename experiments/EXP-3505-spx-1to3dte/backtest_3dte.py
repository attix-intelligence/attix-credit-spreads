"""
EXP-3505: SPX 3 DTE Iron Condors

Strategy:
- Entry: Mon/Wed 9:45 AM
- Exit: Thu/Mon at open (3 day hold)
- Strikes: 30Δ put/call (wider for 3 days)
- Frequency: 2×/week (~100 days/year)
- Test period: 2023-2025

This tests if 3 DTE provides best risk/reward with wider strikes.
"""
...
class SPX3DTEBacktest:
    """Backtest SPX 3 DTE iron condors."""
    ...
    def __init__(self, start_date: str, end_date: str):
        ...
        cache_dir = Path(__file__).parent.parent.parent / "data" / "cboe_complete" / "spx" / "3dte"
        self.provider = CBOECSVProvider(cache_dir=str(cache_dir))
        
        # Strategy params
        self.target_delta = 0.30  # Wider strikes for 3 days
        self.wing_width = 50  # $50 wide spreads
        self.contracts = 1
        self.entry_frequency = 3  # Enter every 3 days (Mon/Thu pattern)
    ...
    def run_backtest(self):
        """Run the backtest."""
        logger.info("=" * 80)
        logger.info("SPX 3 DTE IRON CONDOR BACKTEST")
        logger.info("=" * 80)
        logger.info(f"Period: {self.start_date} to {self.end_date}")
        logger.info(f"Target Delta: {self.target_delta}")
        logger.info(f"Wing Width: ${self.wing_width}")
        logger.info(f"Entry Frequency: Every {self.entry_frequency} days")
        ...
        # Backtest every Nth day
        for i in range(0, len(trading_days) - 3, self.entry_frequency):
            entry_date = trading_days[i]
            
            # Exit is 3 trading days later
            if i + 3 >= len(trading_days):
                continue
            exit_date = trading_days[i + 3]
            
            # Progress
            if i % 50 == 0:
                logger.info(f"Processing day {i}/{total_days}: {entry_date}")
            ...
            # Find strikes (3 DTE on entry)
            put_strike, call_strike = self.find_strikes_for_delta('SPX', entry_date, 3, self.target_delta)
            ...
            # Get entry credits
            put_credit = self.get_spread_price('SPX', entry_date, 3, put_strike, put_long, 'put')
            call_credit = self.get_spread_price('SPX', entry_date, 3, call_strike, call_long, 'call')
    ...
    def analyze_results(self) -> dict:
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info("BACKTEST RESULTS - SPX 3 DTE IRON CONDORS")
        logger.info("=" * 80)
...
def main():
    ...
    if backtest.trades:
        ...
        trades_df.to_csv(output_dir / '3dte_trades.csv', index=False)
        logger.info(f"Saved trades to {output_dir / '3dte_trades.csv'}")
    
    # Save equity curve
    if backtest.equity_curve:
        equity_df = pd.DataFrame(backtest.equity_curve)
        equity_df.to_csv(output_dir / '3dte_equity.csv', index=False)
        logger.info(f"Saved equity curve to {output_dir / '3dte_equity.csv'}")
    
    # Save summary
    import json
    with open(output_dir / '3dte_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    logger.info(f"Saved results to {output_dir / '3dte_results.json'}")
