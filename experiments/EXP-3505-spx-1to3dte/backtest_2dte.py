"""
EXP-3505: SPX 2 DTE Iron Condors

Strategy:
- Entry: 9:45 AM every other day
- Exit: 2 days later at open (2 day hold)
- Strikes: 25Δ put/call
- Frequency: 3×/week (~125 days/year)
- Test period: 2023-2025

This tests if 2 DTE provides better risk/reward than 1 DTE.
"""
...
class SPX2DTEBacktest:
    """Backtest SPX 2 DTE iron condors."""
    ...
    def __init__(self, start_date: str, end_date: str):
        ...
        cache_dir = Path(__file__).parent.parent.parent / "data" / "cboe_complete" / "spx" / "2dte"
        ...
        self.contracts = 1
        self.entry_frequency = 2  # Enter every N days
    ...
    def run_backtest(self):
        """Run the backtest."""
        logger.info("=" * 80)
        logger.info("SPX 2 DTE IRON CONDOR BACKTEST")
        logger.info("=" * 80)
        ...
        for i in range(0, len(trading_days) - 2, self.entry_frequency):
            entry_date = trading_days[i]
            
            # Exit is 2 trading days later
            if i + 2 >= len(trading_days):
                continue
            exit_date = trading_days[i + 2]
            ...
            # Find strikes (2 DTE on entry)
            put_strike, call_strike = self.find_strikes_for_delta('SPX', entry_date, 2, self.target_delta)
            ...
            # Get entry credits
            put_credit = self.get_spread_price('SPX', entry_date, 2, put_strike, put_long, 'put')
            call_credit = self.get_spread_price('SPX', entry_date, 2, call_strike, call_long, 'call')
    ...
    def analyze_results(self) -> dict:
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info("BACKTEST RESULTS - SPX 2 DTE IRON CONDORS")
        logger.info("=" * 80)
...
def main():
    ...
    if backtest.trades:
        ...
        trades_df.to_csv(output_dir / '2dte_trades.csv', index=False)
        logger.info(f"Saved trades to {output_dir / '2dte_trades.csv'}")
    
    # Save equity curve
    if backtest.equity_curve:
        equity_df = pd.DataFrame(backtest.equity_curve)
        equity_df.to_csv(output_dir / '2dte_equity.csv', index=False)
        logger.info(f"Saved equity curve to {output_dir / '2dte_equity.csv'}")
    
    # Save summary
    import json
    with open(output_dir / '2dte_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    logger.info(f"Saved results to {output_dir / '2dte_results.json'}")
